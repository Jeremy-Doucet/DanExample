...
export function create(req: express.Request, res: express.Response, next: Function) {
  ...
  c.save((err, comment) => {
    ...
    Blog.update({ _id: req.params.blogId }, { $push: { Comments: c._id }}, (err, result: any) => {
      if (err) return next(err);
      if (result.nModified === 0 ) return next({ status: 404, message: `Could not update blog, with an id of: ${req.params.id}`});
      res.json(comment);
    });
  });
}
